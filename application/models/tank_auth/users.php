...
class Users extends CI_Model
{
  ...
  function get_user_name_by_id($user_id)
  {
	...
	// test [OK]
	function get_folder_max_count_by_id($user_id, $activated)
	{
		$this->db->select('folder_max_count');
		$this->db->where('id', $user_id);
		$this->db->where('activated', $activated ? 1 : 0);

		$query = $this->db->get($this->table_name);
		if ($query->num_rows() == 1){ return $query->row()->folder_max_count;}
		return NULL;
	}

	function get_email_by_id($user_id, $activated)
	{
